...
public class Fire_andMonsters_sighted_Event_Test {
    ...
    public void fireEventTest() throws InterruptedException {
        ...
        try {
	        ...
	        assertTrue(eventstrace.stream().filter(m -> 
        	EventsProducer.isFireInSightEvent(m) && m.getArgs().length > 0 && m.getArgs()[0].equals(0)
        	)
        	.count() > 0) ;
    ...
    public void monsterEventTest2() throws InterruptedException {
        ...
        try {
	        ...
	        var G = GoalLib.atBGF("button0", 0.5f, false) ;
	        ...
	        assertTrue(eventstrace.stream().filter(m -> 
	        	EventsProducer.isMonstersInSightEvent(m) && m.getArgs().length > 0 && m.getArgs()[0].equals(1)
	        	)
	        	.count() > 0) ;
	        
	        assertTrue(eventstrace.stream().filter(m -> 
        	EventsProducer.isMonstersInSightEvent(m) && m.getArgs().length > 0 && m.getArgs()[0].equals(0)
        	)
        	.count() > 0) ;
